...
@bot.command(pass_content=True, brief='Shows users balance', aliases=['bal'])
async def balance(ctx):
  USER_ID = ctx.message.author.id
  USER_NAME = str(stx.message.author)
...
@bot.command(pass_context=True, brief='list top 10 bank accounts', aliases=['top10'])
async def top10(ctx):
